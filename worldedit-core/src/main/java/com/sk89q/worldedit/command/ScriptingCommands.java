...
public class ScriptingCommands {
    ...
    @Command(
        aliases = { "cs" },
        usage = "<имя_файла> [аргументы...]",
        desc = "Выполнить CraftScript",
        min = 1,
        max = -1
    )
    ...
    public void execute(Player player, LocalSession session, CommandContext args) throws WorldEditException {
        ...
        if (!player.hasPermission("worldedit.scripting.execute." + name)) {
            player.printError("У вас нет разрешения использовать этот скрипт.");
            return;
        }
    ...
    @Command(
        aliases = { ".s" },
        usage = "[args...]",
        desc = "Выполнить последний CraftScript",
        min = 0,
        max = -1
    )
    @CommandPermissions("worldedit.scripting.execute")
    ...
    public void executeLast(Player player, LocalSession session, CommandContext args) throws WorldEditException {
        ...
        if (lastScript == null) {
            player.printError("Используйте /cs с именем первого скрипта.");
            return;
        }
